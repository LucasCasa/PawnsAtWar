...
public class ScheduleServiceImpl implements ScheduleService {
  ...
  @Override
  public void returnTask(User u, Army a, Point from) {
    Calendar c = Calendar.getInstance();
    int time = (int) (Math.log10(Math.pow(a.getPosition().getX() - from.getX(), 2) + Math.pow(a.getPosition().getY() - from.getY(), 2) + 1) * 60);
    c.add(Calendar.SECOND, time);
    Date d = c.getTime();
    Alert alert = as.createAlert(u, getReturnAlertMessage(from), d, AlertType.RETURN.toString(), from, a.getIdArmy(), null);
    setReturnTask(a.getIdArmy(), alert, d);
  }
  ...
  public void resumeTask(Alert a) {
    ...
    if (s.equals(AlertType.BUILD.toString())) {
    ...
    } else if (s.equals(AlertType.MOVE.toString()) || s.equals(AlertType.SPLIT.toString())) {
      ...
      setReturnTask(a.getParam1(), a, a.getDate());
    }
  }
  private void setReturnTask(int armyId, Alert alert, Date d) {
    ...
    Runnable buildRunnable = new Runnable() {
      ...
      public void run() {
        ...
        if (d != null && d.getAvailable()) {
          ...
          String prefixW;
          String prefixD;
          if (defenderP > attackerP) { //DEFENDER WON
            ...
            prefixW = "d";
            res = 1;
          } else if (attackerP > defenderP) { //ATTACKER WON
  ...
  private String getReturnAlertMessage(Point from) {
    Locale l = LocaleContextHolder.getLocale();
    Object[] params = new Object[2];
    params[0] = from.getX();
    params[1] = from.getY();
    return messageSource.getMessage("alert.return", params, l);
  }
  ...
  private void sendMailToDefender(User d){
    Locale l = d.getLocale() == null ? LocaleContextHolder.getLocale() : new Locale(d.getLocale());
    mailService.sendEmail(d.getEmail(),messageSource.getMessage("email.subject", null, l), messageSource.getMessage("email.body", null, l));
  }
}
